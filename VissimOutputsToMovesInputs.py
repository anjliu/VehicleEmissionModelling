...
def getOpMode(inter,direc,movements,fzp,lsa):
    ...
    allLinks=ts.getInterLinks(interspecs,direc,movements)
    
    sc=interspecs['SignalController'] # signal controller of intersection
    sg=interspecs[direc]['SignalGroup']['thru'] # signal group of interest
    
    # get link lengths
    lengthMap=ts.getLengthMap(allLinks)
    lengthtotal=sum(lengthMap)
    
    ### Set up for result saving
    
    # create dataframes for MOVES inputs
    ...
    cycles=range(len(tg))
    for c in cycles:
        fzpintc=fzp.loc[(fzp['sec']>=tg[c]) & (fzp['sec']<=tr[c])] # fzp for this intersection-approach and this cycle
        vehpercycle[c]=fzpintc['no'].unique() # vehicle numbers in this cycle
        fzppercycle[c]=fzpintc
